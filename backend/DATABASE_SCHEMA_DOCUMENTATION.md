...
|---|---|---|
| **User Domain** | USERS | User identity and goal |
...
- Overall tier is computed dynamically at roadmap generation time — not stored in DB
...
**Purpose:** Core user identity table. Stores user ID and goal role.
...
- Overall tier is not stored here — it is derived dynamically per-skill at roadmap generation time from `USER_SKILL_ASSESSMENTS`
...
- `proficiency_level` per skill drives which content level is assigned in roadmap — each skill is independent
- `weak_topics` drives which targeted gap resources are shown alongside each roadmap step
- One row per user per skill — clean and queryable
- Certificate tier is derived from these rows at roadmap completion time — not stored separately
...
- `skill` and `level` enable per-skill level tracking — each skill progresses independently
...
Roadmap generated per-skill — each skill gets content at its own detected level. No global tier applied.

| Step | Skill | Content Level | Status | Targeted Resources |
|---|---|---|---|---|
| 1 | Python | Intermediate | active | decorators resource |
| 2 | SQL | Beginner | locked | joins, grouping resources |
| 3 | FastAPI | Beginner | locked | none yet assessed |
| 4 | Docker | Beginner | locked | none yet assessed |
| 5 | Git | Beginner | locked | none yet assessed |
...
| Step | Skill | Content Level | Status |
|---|---|---|---|
...
**Step 8 — All steps completed — final assessment signaled**
```
generate-path response:
  is_path_finished: true
  next_action:      final_assessment
  certificate_tier: Beginner
  message:          All courses complete. Take final assessment to earn your Beginner certificate.
```

Certificate tier is derived at completion time from proficiency levels in USER_SKILL_ASSESSMENTS:
- All skills Advanced → Advanced certificate
- All skills Intermediate or above → Intermediate certificate
- Any skill Beginner → Beginner certificate